function Q_DT_MMAPK_ParsePara(D0,D0_name,D,D_name)
%Q_DT_MMAPK_ParsePara checks the validity of the input matrices D0, D as a
%representation of a Discrete-Time MMAP[K]. D{i} holds the mxm matrix D_i.

% check numeric 
if (~isnumeric(D0))
    error('MATLAB:Q_DT_MMAPK_ParsePara:InvalidInput',...
...
if (min(min(D0)) < -10^(-14))
    error('MATLAB:Q_DT_MMAPK_ParsePara:InvalidInput',...
        'The matrix %s contains negative data', D0_name);
end
for i=1:K
...
if (max(sum(D0,2)) > 1+10^(-14))
    error('MATLAB:Q_DT_MMAPK_ParsePara:InvalidInput',...
        'The matrix %s has to be substochastic', D0_name);
end    
Dsum = D0;
for i = 1:K
    Dsum = Dsum + D{i};
end
if (max(sum(Dsum,2)) > 1+10^(-14))||(min(sum(Dsum,2)) < 1-10^(-14))
    error('MATLAB:Q_DT_MMAPK_ParsePara:InvalidInput',...
        'The matrix %s+%s_1+...+%s_K has to be stochastic', D0_name,D_name,D_name);
end    
