function AvgTableNet = example_cacheModel_6_sub1(S,N,pHit)
network = Network('network');

mainDelay = Delay(network, 'MainDelay');
hitDelay = Queue(network, 'HitQ', SchedStrategy.FCFS);
missDelay = Queue(network, 'MissQ', SchedStrategy.FCFS);
...
P = {};
P{1,1} = [0,pHit(1),1-pHit(1); 1,0,0; 1,0,0];
P{1,2} = [0,0,0; 0,0,0; 0,0,0];
P{2,2} = [0,pHit(2),1-pHit(2); 1,0,0; 1,0,0];
