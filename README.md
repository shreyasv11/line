...
LINE is a hybrid tool for performance and reliability analysis of complex systems based on queueing theory and stochastic modeling. Using LINE, it is possible to decouple the model description from the solvers used for its solution. LINE acts as a bridge, offering model-to-model transformations that automatically translate the model specification to the chosen target solver.
