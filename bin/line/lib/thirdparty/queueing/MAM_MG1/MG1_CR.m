...
if (strfind(options.Mode,'ShiftPWCR')>0)
    ...
    if (strcmp(options.ShiftType,'one'))
        [D,drift]=MG1_Shift(D);
    else
        [D,drift,tau,v]=MG1_Shift_More(D,'tau');
    end   
end
...
while(1 && numit < options.MaxNumIt)
    ...
    if (strcmp(options.Mode,'PWCR') || strcmp(options.Mode,'DCR'))
        ...
        Rj=Rnewj;
        if (norm(Anew(1:m,1:m))<options.EpsilonValue || size(Ahatnew,1)==m || max(sum(eye(m)-D(1:m,:)*inv(eye(m)-Ahatnew(1:m,:)))) < options.EpsilonValue)
            G=D(1:m,:)*inv(eye(m)-Ahatnew(1:m,:));
            break
        end
    else 
        A1norm=norm(Anew(1:m,:),inf);
        sumnorm=0;
        for i=2:size(Anew,1)/m-1
            sumnorm=sumnorm+norm(Anew(i*m+1:(i+1)*m,:),inf);
        end 
        if (min(A1norm,sumnorm)<options.EpsilonValue)
            if (lastiter==1)
                G=D(1:m,:)*inv(eye(m)-Ahatnew(1:m,:));
                break;
            end  
