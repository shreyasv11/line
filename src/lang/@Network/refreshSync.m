...
for ind=1:qn.nnodes
    for r=1:qn.nclasses
        if qn.isstation(ind) && qn.phases(qn.nodeToStation(ind),r)> 1
            ...
            sync{end,1}.active{1} = Event(Event.PHASE, ind, r);
            sync{end,1}.passive{1} = Event(Event.LOCAL, local, r, 1.0);
        end
        if qn.isstateful(ind)
            if qn.nodetype(ind) == NodeType.Cache
                if ~isnan(qn.varsparam{ind}.pref{r}) % class can read
                    sync{end+1,1}.active{1} = Event(Event.READ, ind, r);
                    sync{end,1}.passive{1} = Event(Event.READ, local, r, 1.0);
                end
            end
            ...
            for jnd=1:qn.nnodes
                if qn.isstateful(jnd)
                    ...
                    for s=1:nclasses
                        ...
                        if p > 0
                            sync{end+1,1} = struct('active',cell(1),'passive',cell(1));
                            sync{end,1}.active{1} = Event(Event.DEP, ind, r);
                            switch qn.routing(ind,s)
                                case {RoutingStrategy.ID_RR, RoutingStrategy.ID_JSQ}
                                    sync{end,1}.passive{1} = Event(Event.ARV, jnd, s, @(state_before, state_after) at(self.qn.rtfun(state_before, state_after), (isf-1)*nclasses+r, (jsf-1)*nclasses+s));
                                otherwise
                                    sync{end,1}.passive{1} = Event(Event.ARV, jnd, s, self.qn.rt((isf-1)*nclasses+r, (jsf-1)*nclasses+s));
                            end
                        end
                    end
                end
